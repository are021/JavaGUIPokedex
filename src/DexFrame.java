...
public class DexFrame extends JFrame implements ActionListener {
	...
	String[] pokes;
	ImageIcon pikachu = new ImageIcon("./pkmnimages/fixed.png");
	DexFrame(String[] pkmn, HashMap<String, String[]> dex){
		
		dexEntries = dex;
		pokes = pkmn;
		imageArray = makeImageArray();
	...
	public ImageIcon[] makeImageArray() {
		ImageIcon[] temp = new ImageIcon[151];
		String filename;
		for (int i = 0; i < pokes.length; i++) {
			if (pokes[i].equals("Mr. Mime")) {
				System.out.println(pokes[i]);
				filename = "./pkmnimages/" + ("mr-mime") + ".png";
				temp[i] = new ImageIcon(filename);
				continue;
			}
			filename = "./pkmnimages/" + (pokes[i].toLowerCase()) + ".png";
